...
export const IntelligentProfileTab: React.FC<Props> = ({ student, user, onNavigateToIncluiLab: _onNavigateToIncluiLab }) => {
  ...
  // Visual principal da aba = layout HTML premium do perfil. Este toggle troca
  // para a prévia A4 do documento (IntelligentProfilePdfPreview) sem tocar na
  // geração de PDF/Word.
  ...
  return (
    <div className="min-h-screen bg-[#F6F4EF] font-sans text-slate-800 pb-12 -mx-6 px-6 print:bg-white print:mx-0 print:px-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {profile && data && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-white text-[#1F4E5F] hover:bg-[#EEF5F8] border border-[#C5DDE7] rounded-xl font-semibold text-sm transition-all"
            >
              {showDocPreview
                ? <><ArrowLeft size={15} /> Voltar ao perfil</>
                : <><FileText size={15} /> Visualizar documento</>}
            </button>
          )}
        </div>
      ...
      {/* ── PERFIL — visual principal (layout HTML premium) ────────────────── */}
      {/* O toggle "Visualizar documento" troca para a prévia A4 do PDF real
          (IntelligentProfilePdfPreview) — a geração de PDF/Word não muda. */}
      {!isGenerating && profile && data && showDocPreview && (
