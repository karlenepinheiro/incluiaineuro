...
export function StudentCompletionBadge({ student }: { student: Student }) {
  ...
  const { isComplete, missingFields } = getStudentCompletionStatus(student);
  if (isComplete) return <span className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs bg-emerald-50 text-emerald-800 border border-emerald-200"><CheckCircle2 size={12} />Cadastro completo</span>;
  return <span onClick={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()}>
    <button ref={trigger} type="button" popoverTarget={id} aria-controls={id} aria-haspopup="dialog"
      className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs bg-red-50 text-red-800 border border-red-200 focus-visible:outline focus-visible:outline-2 focus-visible:outline-red-800">
      <AlertCircle size={12} />Cadastro incompleto · {missingFields.length} campos ausentes
    </button>
    <div id={id} popover="auto" onToggle={event => {
      ...
      <p className="font-semibold">Campos essenciais ausentes</p>
      <ul className="list-disc pl-5 mt-2 space-y-1">{missingFields.map(field => <li key={field.key}>{field.label}</li>)}</ul>
