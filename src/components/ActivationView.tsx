...
export const ActivationView: React.FC<Props> = ({
  ...
  const runCheck = async (emailToCheck: string) => {
    ...
    try {
      ...
      if (isLoggedIn && result.purchase_id) {
        await doActivation(result.purchase_id, result.plan_code);
      } else {
        setState('approved');
      }
    } catch (e: any) {
  ...
  // ── Ativação ────────────────────────────────────────────────────────────────
  const doActivation = async (pid: string, plan?: string | null) => {
    try {
      const result = await activatePurchaseForUser(pid);
      if (result.ok) {
        setState('activated');
        onActivated(result.plan ?? plan ?? 'PRO');
      } else if (result.reason === 'already_activated') {
        // Compra já foi ativada antes — trata como sucesso
        setState('activated');
        onActivated(plan ?? 'PRO');
      } else {
        setState('error');
        setErrorMsg('Não foi possível ativar o plano. Entre em contato com o suporte.');
      }
    } catch (e: any) {
      setState('error');
      setErrorMsg(e?.message ?? 'Erro ao ativar plano.');
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (purchaseId) {
        await doActivation(purchaseId, planCode);
      } else {
        setState('activated');
        onActivated(planCode ?? 'PRO');
      }
    } catch (e: any) {
      setRegError(e?.message ?? 'Erro ao criar conta.');
      setState('approved'); // volta ao form
    } finally {
      setRegLoading(false);
    }
  };
  ...
  // ── Helpers visuais ─────────────────────────────────────────────────────────
  const planLabel = planCode === 'MASTER' ? 'Premium' : planCode === 'PRO' ? 'Pro' : planCode ?? 'Pago';
      ...
      <div style={{ width: '100%', maxWidth: 460 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: `1px solid ${C.border}`, padding: '32px 28px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          {state === 'registering' && (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <Loader2 size={40} style={{ color: C.petrol, animation: 'spin 1s linear infinite', margin: '0 auto 16px' }} />
              <p style={{ color: '#64748b', fontSize: 15 }}>Criando sua conta e ativando o plano…</p>
              <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
            </div>
          )}
          ...
          {state === 'approved' && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                ...
                <div>
                  <p style={{ fontWeight: 700, color: '#15803d', fontSize: 15, margin: 0 }}>Pagamento confirmado!</p>
                  <p style={{ color: '#64748b', fontSize: 13, margin: 0 }}>Plano {planLabel} — {email}</p>
                </div>
              </div>
              ...
              >
                {regLoading
                  ...
                  : <>Criar conta e ativar plano {planLabel} <ArrowRight size={16} /></>
                }
                <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
              </button>
          ...
          {state === 'awaiting_email' && (
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              ...
              <p style={{ color: '#64748b', fontSize: 14, lineHeight: 1.6, marginBottom: 20 }}>
                ...
                Clique no link para confirmar e seu plano <strong>{planLabel}</strong> será ativado automaticamente ao fazer login.
              </p>
