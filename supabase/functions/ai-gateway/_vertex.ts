...
/**
 * Leitura multipágina (27/08/2026, restaurada em 06/09/2026 — regressão C-1):
 * `images`, quando presente, tem precedência sobre `imageBase64` — várias
 * páginas do MESMO documento viram várias partes `inlineData` na MESMA
 * requisição multimodal (Gemini já suporta múltiplas partes de imagem por
 * `contents[0].parts`; nada no lado do provider precisou mudar de
 * arquitetura, só compor mais partes). Cada página só recebe o rótulo
 * textual "Página N" quando há mais de uma — com uma única imagem
 * (`imageBase64` ou `images` de tamanho 1), o payload enviado ao Gemini é
 * BYTE A BYTE idêntico ao de antes desta mudança.
 *
 * `pageNumbers` (correção de numeração): quando informado, cada parte é
 * rotulada com o número REAL da página no documento original — evita
 * renumerar como "Página 1, Página 2..." um conjunto de imagens do qual
 * páginas do meio (ex.: em branco) foram removidas. Ausente → mesmo fallback
 * por posição de antes. Ver _multiPageParts.ts (testado isoladamente).
 */
