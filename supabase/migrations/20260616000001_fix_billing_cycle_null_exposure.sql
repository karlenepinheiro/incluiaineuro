-- ============================================================
-- 20260616000001_fix_billing_cycle_null_exposure.sql
--
-- SPRINT P1.3-B — Corrigir mascaramento de billing_cycle NULL
-- em v_ceo_subscriber_health e views dependentes.
--
-- PROBLEMA:
--   v_ceo_subscriber_health aplicava COALESCE(..., 'monthly') no
--   billing_cycle de subscriptions, fazendo assinaturas sem ciclo
--   definido aparecerem como 'monthly' no painel CEO.
--   Isso escondia anomalias e impedia diagnóstico correto.
--
-- CORREÇÃO:
--   1. latest_sub CTE: remove COALESCE fallback — expõe NULL
--      quando billing_cycle não está preenchido.
--   2. raw_health CTE: CASE não mais faz fallback para 'monthly'
--      — retorna NULL quando o ciclo é desconhecido.
--   3. financial_inconsistency: agora inclui billing_cycle IS NULL
--      como condição explícita de inconsistência financeira.
--
-- IMPACTO:
--   - Painel CEO passará a exibir billing_cycle = NULL para
--     assinaturas sem ciclo definido (em vez de 'monthly' falso).
--   - Essas assinaturas gerarão finding FINANCIAL_INCONSISTENCY
--     de severidade 'warn', visível na aba de integridade CEO.
--   - Contagens de pro_monthly_count/pro_annual_count/etc. em
--     v_ceo_financial_kpis excluirão ciclos desconhecidos, evitando
--     MRR inflado por dados inválidos.
--
-- NÃO ALTERA:
--   - Dados de assinatura (nenhum UPDATE/INSERT/DELETE)
--   - Créditos, saldos, usuários
--   - Tabelas fora das views abaixo
-- ============================================================

-- Dependências em cadeia:
--   v_ceo_subscriber_health
--     └─ v_ceo_wallet_divergences
--     └─ v_ceo_integrity_findings
--           └─ v_ceo_subscribers
--           └─ v_ceo_financial_kpis
-- Recriamos todas na ordem correta.

DROP VIEW IF EXISTS public.v_ceo_financial_kpis   CASCADE;
DROP VIEW IF EXISTS public.v_ceo_subscribers       CASCADE;
DROP VIEW IF EXISTS public.v_ceo_integrity_findings CASCADE;
DROP VIEW IF EXISTS public.v_ceo_wallet_divergences CASCADE;
DROP VIEW IF EXISTS public.v_ceo_subscriber_health  CASCADE;

-- ============================================================
-- 1. v_ceo_subscriber_health  (VIEW BASE — CORRIGIDA)
-- ============================================================
...
latest_sub AS (
  SELECT DISTINCT ON (s.tenant_id)
    ...
    s.next_due_date,
    -- FIX: não usar COALESCE(..., 'monthly') — expõe NULL quando billing_cycle é nulo/vazio
    NULLIF(lower(trim(s.billing_cycle)), '') AS billing_cycle,
...
raw_health AS (
  SELECT
    ...
    -- FIX: retorna NULL explicitamente quando billing_cycle é desconhecido
    -- Antes: ELSE 'monthly' (mascarava anomalias como mensais)
    CASE
      WHEN ls.billing_cycle IN ('annual', 'monthly') THEN ls.billing_cycle
      ELSE NULL
...
SELECT
  ...
  -- next_due_date: COALESCE igual ao original (raw next_due_date não exposto separadamente)
  ...
  eh.billing_cycle,   -- NULL quando ciclo desconhecido (não mais 'monthly' falso)
  eh.plan_code,
  eh.credits_limit,
  eh.student_limit,
  eh.plan_price_brl,
  ...
  -- FIX: billing_cycle IS NULL agora é detectado como inconsistência financeira
  -- Antes: billing_cycle NOT IN ('monthly', 'annual') com NULL retornava NULL (invisível)
  (
    eh.plan_code <> 'FREE'
    ...
    AND (
      ...
      OR eh.billing_cycle IS NULL                           -- explicitamente flag NULL
...
-- ============================================================
-- 2. v_ceo_wallet_divergences  (sem alteração lógica)
-- ============================================================
...
-- ============================================================
-- 3. v_ceo_integrity_findings  (sem alteração lógica)
-- ============================================================
    ...
    FROM (
      VALUES
        ...
        (
          ...
          'Assinatura paga sem preço válido, sem renovação, billing_cycle NULL ou ciclo inválido.',
          80,
...
-- ============================================================
-- 4. v_ceo_subscribers  (sem alteração lógica)
-- ============================================================
...
SELECT
  ...
  h.user_cpf,
  h.plan_code,
  h.billing_cycle,          -- NULL quando ciclo desconhecido
  h.subscription_status,
  h.next_due_date,
  h.billing_provider,
  h.provider_sub_id,
...
-- ============================================================
-- 5. v_ceo_financial_kpis  (sem alteração lógica)
-- ============================================================
...
subscriber_findings AS (
  SELECT * FROM public.v_ceo_integrity_findings
...
extra_revenue AS (
  ...
  LEFT JOIN public.users u    ON lower(trim(u.email)) = lower(trim(kp.email))
...
SELECT
  ...
  -- Nota: subscriptions com billing_cycle NULL não entram em nenhum bucket acima (correto)
  (SELECT COUNT(*) FROM paid_active WHERE billing_cycle IS NULL)::int                  AS unknown_cycle_count,
